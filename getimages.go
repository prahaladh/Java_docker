...
func GetLatestPod(pods []corev1.Pod) *corev1.Pod {
    ...
    // Return the latest pod (the first one after sorting)
    return &pods[0]
}

// PrintPodImage prints the image of the specified container (or the first container if none is specified)
func PrintPodImage(pod *corev1.Pod, containerName string) {
    if pod == nil {
        fmt.Println("No pods found.")
        return
    }

    fmt.Printf("Latest Pod Name: %s\n", pod.Name)
    for _, container := range pod.Spec.Containers {
        if containerName == "" || container.Name == containerName {
            fmt.Printf("Container Name: %s, Image: %s\n", container.Name, container.Image)
            return // Print only one image
        }
    }

    fmt.Println("Specified container not found in the latest pod.")
}

func main() {
    // Command-line arguments for namespace, service name, and container name
    namespace := flag.String("namespace", "default", "Kubernetes namespace")
    serviceName := flag.String("service", "", "Kubernetes service name")
    containerName := flag.String("container", "", "Kubernetes container name (optional)")
    flag.Parse()
    ...
    // Get the latest pod
    latestPod := GetLatestPod(pods)

    // Print the image of the specified container (or the first container)
    PrintPodImage(latestPod, *containerName)
}
