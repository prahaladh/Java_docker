...
func GetDeployedImageForContainer(pods []corev1.Pod, containerName string) {
...
func main() {
    ...
    containerName := flag.String("container", "", "Container name to fetch the image details")
    flag.Parse()

    if *serviceName == "" || *containerName == "" {
        log.Fatal("Service name and container name must be provided")
    }
    ...
    // Get deployed image for the specified container
    GetDeployedImageForContainer(pods, *containerName)
}
