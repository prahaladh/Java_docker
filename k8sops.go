...
func main() {
	...
	overallStatus := "Running" // Default status

	for _, pod := range pods.Items {
		// Check for non-running pod phase
		...
		// Check container statuses for restarts or crash loops
		for _, containerStatus := range pod.Status.ContainerStatuses {
			if containerStatus.RestartCount > 0 {
				fmt.Printf("Pod %s has container %s that restarted %d times\n", pod.Name, containerStatus.Name, containerStatus.RestartCount)
			}
			// Check for CrashLoopBackOff
			if containerStatus.State.Waiting != nil && containerStatus.State.Waiting.Reason == "CrashLoopBackOff" {
				overallStatus = "CrashLoopBackOff"
				break
			}
		}
	}
